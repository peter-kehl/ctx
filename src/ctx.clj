...
(defmacro tags [] ;TODO `(def ctx-strict true) ;in user's namespace, not in ctx/ - so that code generated by (scope) macro can access it - TODO how to access it?
  )
...
#_(defn reader-bottom "Capture a scope bottom. Don't call directly. See level()." [level-macro-or-call]
  {:pre [(list? level-macro-call) (= (first level-macro-or-call) `level) (= (count level-macro-or-call) 2)
    *collected-symbols*]}
  (try
    (list `level reader-bottom-token))
    (finally (set! *collected-symbols* false) ;TODO if strict, set to {}
  ))
